...
const choix_liste_departements1 = view(Inputs.select(noms_departements, { label: "Liste des départements"}));
const choix_liste_essences1 = view(Inputs.select(noms_essences, { label: "Liste des essences"}));
const choix_list_annee1 = view(Inputs.select(annees, { label: "Choix de l'année"}));
```

```js
/*Affichage des 3 listes de paramètres dynamiques*/ 
const choix_liste_departements2 = view(Inputs.select(noms_departements, { label: "Liste des départements"}));
const choix_liste_essences2 = view(Inputs.select(noms_essences, { label: "Liste des essences"}));
const choix_list_annee2 = view(Inputs.select(annees, { label: "Choix de l'année"}));
```
